...
export function useCompanyBrainRoots(): PeResource<CompanyBrainSearchResult[]> {
  ...
  const load = useCallback(async () => {
    const result = await centropyPost<{ results: CompanyBrainSearchResult[] }>("company-brain/roots", { query: "", limit: 50 })
...
export function useCompanyBrainProjection(root: PeWorldRootRef | null): PeResource<CompanyBrainProjection> {
  ...
  const load = useCallback(async () => {
    const result = await centropyPost<CompanyBrainProjection>("company-brain/projection", { root: { entityType: rootType, entityId: rootId } })
...
export function useSemanticActivity(root: PeWorldRootRef | null, limit = 180): PeResource<SemanticActivityProjection> {
  ...
  const rootId = root?.entityId ?? null
  const load = useCallback(async () => {
    const result = await centropyPost<SemanticActivityProjection>("semantic-activity", { root: { entityType: rootType, entityId: rootId }, limit })
...
export function useWorkforceStatus(enabled: boolean): PeResource<WorkforceStatusProjection> {
  const { session } = useCentropyAuth()
  const load = useCallback(async () => (await centropyGet<{ data: WorkforceStatusProjection }>("read-models/workforce-status")).data, [])
...
export function useAttentionQueue(): PeResource<AttentionQueueResult> {
  const { session } = useCentropyAuth()
  const load = useCallback(async () => (await centropyGet<{ data: AttentionQueueResult }>("read-models/attention", { limit: "100" })).data, [])
  ...
  const load = useCallback(async (): Promise<FirmDealProjectionSet> => {
    const results = await Promise.allSettled(dealRoots.map(async (root) => {
      const [projection, activity] = await Promise.allSettled([
        centropyPost<CompanyBrainProjection>("company-brain/projection", { root }),
        centropyPost<SemanticActivityProjection>("semantic-activity", { root, limit: 1 }),
      ])
...
export async function loadCompanyBrainObject<T>(operation: "object" | "provenance" | "history" | "evidence-lineage" | "decision-lineage" | "traverse", root: PeWorldRootRef, ref: CompanyBrainObjectRef): Promise<T> {
  return centropyPost<T>(`company-brain/${operation}`, { root, ref, ...(operation === "history" ? { limit: 50 } : {}), ...(operation === "traverse" ? { depth: 4, direction: "both", limit: 250 } : {}) })
}