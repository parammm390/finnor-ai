      ...
      <div className="w-full max-w-sm rounded-2xl border border-[color:var(--j-border)] bg-slate-950 p-6">
        ...
        {recoverySession ? (
          ...
          <form onSubmit={requestReset} className="space-y-3">
            <div>
              <label htmlFor="centropy-reset-email" className="mb-1 block j-fs-micro font-bold uppercase tracking-widest text-[color:var(--j-text-faint)]">Email</label>
              <input id="centropy-reset-email" type="email" required autoFocus value={email} onChange={(e) => setEmail(e.target.value)} className="h-10 w-full rounded-xl border border-white/12 bg-slate-900 px-3 j-fs-sm text-white focus:border-[color:var(--j-border-hot)] focus:outline-none" placeholder="you@example.com" />
            </div>
        ...
        <Link href="/centropy/login" className="mt-4 block text-center j-fs-micro text-[color:var(--j-text-faint)] hover:text-white">Back to sign in</Link>
      </div>
    </div>
  )
}
