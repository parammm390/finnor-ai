...
export function LoginForm() {
  ...
  async function submit(e: React.FormEvent): Promise<void> {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : "CENTROPY sign-in is unavailable.")
    } finally {
      setBusy(false)
    }
  }
    ...
    <div className="flex min-h-screen items-center justify-center bg-[#04070f] px-4 text-[color:var(--j-text)]">
      <div className="w-full max-w-sm rounded-2xl border border-[color:var(--j-border)] bg-slate-950 p-6">
        ...
        <form onSubmit={submit} className="space-y-3">
          <div>
            <label htmlFor="centropy-login-email" className="mb-1 block j-fs-micro font-bold uppercase tracking-widest text-[color:var(--j-text-faint)]">
              Email
            </label>
            <input
              id="centropy-login-email"
              ref={emailRef}
              type="email"
              required
              autoFocus
          ...
          <div>
            <label htmlFor="centropy-login-password" className="mb-1 block j-fs-micro font-bold uppercase tracking-widest text-[color:var(--j-text-faint)]">
              Password
            </label>
            <input
              id="centropy-login-password"
              ref={passwordRef}
              type="password"
              required
              value={password}
        ...
        <Link href="/centropy/reset-password" className="mt-3 block text-center j-fs-micro text-[color:var(--j-text-faint)] hover:text-white">
          Forgot your password?
        </Link>
        <Link href="/centropy" className="mt-4 block text-center j-fs-micro text-[color:var(--j-text-faint)] hover:text-white">
          Back to CENTROPY
        </Link>
      </div>
    </div>
