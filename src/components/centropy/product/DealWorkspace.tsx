...
function UnderwritingSection({ projection }: { projection: CompanyBrainProjection }) {
  ...
  const workspace = useProductRequest({ enabled: Boolean(selectedCase), key: caseId, load: () => centropyGet<UnderwritingWorkspace>(`investment-cases/${caseId}/underwriting`), stateFor: (data) => data.runs.length ? "KNOWN" : "KNOWN_EMPTY" })
  ...
  async function explain(run: UnderwritingRun, nodeId: string) {
    ...
    try { setLineage({ key: `${run.id}:${nodeId}`, data: await centropyGet<UnderwritingLineage>(`underwriting/runs/${run.id}/explain`, { nodeId }), error: null }) }
  ...
  async function compare() {
    ...
    try { setComparison(await centropyGet<Record<string, unknown>>("underwriting/runs/diff", { left: runs[1]!.id, right: runs[0]!.id })) }
...
function IcSection({ projection }: { projection: CompanyBrainProjection }) {
  ...
  const workspace = useProductRequest({ enabled: Boolean(selected), key: id, load: () => centropyGet<IcWorkspace>(`private-equity/ic/cases/${id}`), stateFor: (data) => data.readiness.decisionEligible ? "KNOWN" : data.readiness.blockers.length ? "PARTIAL" : "KNOWN", confirmedAt: (data) => data.asOf })
...
export default function DealWorkspace() {
  ...
  return <main className="pw-deal-workspace">
    ...
    <nav className="pw-deal-tabs" aria-label="Deal workspace sections">{DEAL_SECTION_KEYS.map((key) => <Link key={key} data-active={section === key ? "true" : undefined} href={withPeOperatingContext(`/centropy/deals/${projection.root.entityId}/${key}`, rootContext)}>{LABELS[key]}</Link>)}</nav>
