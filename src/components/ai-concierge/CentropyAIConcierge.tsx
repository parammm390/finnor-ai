...
export function CentropyAIConcierge() {
  ...
  async function handleQuickAction(action: string) {
    ...
    if (action === "What does Centropy do?") {
      await waitForThinkingDelay();
      addAssistantMessage({
        content:
  ...
  return (
    <>
      <AnimatePresence>
        {isOpen ? (
          <motion.aside
            ...
            role="dialog"
            aria-label="Centropy AI Concierge"
          >
            ...
            <div className="relative flex min-w-0 flex-1 flex-col">
              <header className="border-b border-white/10 px-4 py-4">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <button
                    type="button"
                    aria-label="Close Centropy AI Concierge"
                    data-cursor="hover"
      ...
      <motion.button
        type="button"
        aria-label={
          isOpen ? "Close Centropy AI Concierge" : "Open Centropy AI Concierge"
        }
        data-cursor="hover"
