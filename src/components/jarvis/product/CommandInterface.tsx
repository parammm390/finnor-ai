...
const JarvisVoiceInput = dynamic(() => import("./JarvisVoiceInput"), { ssr: false })
  ...
  async function execute() {
    ...
    try {
      ...
      if (response.workId && UUID.test(response.workId)) {
        ...
        try {
          const aggregate = (await jarvisGet<{ work: WorkAggregateView }>(`works/${response.workId}`)).work
      ...
      setError(cause instanceof Error ? cause.message : "JARVIS could not submit the governed instruction.")
  ...
  return <div className="pw-command-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
    <section className="pw-command" role="dialog" aria-modal="true" aria-labelledby="pw-command-title" data-stage={stage}>
      ...
      {stage === "draft" ? <form onSubmit={review}>
        ...
        <div className="pw-command__context"><span>PE context</span><strong>{root ? rootLabel ?? humanize(root.entityType) : "No canonical root selected"}</strong>{workspace.config.voiceEnabled ? <JarvisVoiceInput onTranscript={(value) => { setInputChannel("voice"); setInstruction((current) => [current, value].filter(Boolean).join(" ")) }} /> : null}</div>
      ...
      {stage === "review" ? <div className="pw-command__review">
        ...
        <footer><button type="button" onClick={() => setStage("draft")}><ArrowLeft size={13} /> Edit</button><button type="button" onClick={() => void execute()}><ShieldCheck size={14} /> Propose to JARVIS</button></footer>
      </div> : null}
      {stage === "executing" || stage === "verifying" ? <div className="pw-command__progress"><LoaderCircle size={20} /><Status value={stage === "executing" ? "EXECUTING" : "VERIFYING"} /><p>{stage === "executing" ? "JARVIS is planning and applying the existing authority boundary." : "Reading the canonical Work aggregate for receipt or completion proof."}</p></div> : null}
