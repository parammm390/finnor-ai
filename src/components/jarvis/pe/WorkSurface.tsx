...
function WorkDetail({ node }: { node: CompanyBrainNode }) {
  ...
  const aggregate = useProductRequest({ enabled: true, key: id, load: async () => (await jarvisGet<{ work: WorkAggregateView }>(`works/${id}`)).work, stateFor: (data) => data.work.failure ? "CONFLICTING" : "KNOWN" })
  ...
  return <section className="pw-work-detail">
    ...
    <ActionBar><button type="button" onClick={() => operating.inspect(node.inspectionTarget, node.ref)}><Workflow size={13} /> Inspect Work</button><button type="button" onClick={() => command.openCommand({ prompt: `Continue this Work: ${objective}`, target: node.inspectionTarget })}>Continue through JARVIS <ArrowRight size={13} /></button></ActionBar>
