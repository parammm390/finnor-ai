...
export default function PersonalizedHome() {
  ...
  function open(row: (typeof rows)[number]) {
    if (row.workId) {
      router.push(withPeOperatingContext("/jarvis/work", { root: row.root, selectedObject: null, workId: row.workId }), { scroll: false })
      return
    }
    inspect(row)
  }
  ...
  function openDeal(row: (typeof rows)[number]) {
    if (!row.root) return
    router.push(withPeOperatingContext(`/jarvis/deals/${row.root.entityId}/overview`, { root: row.root, selectedObject: null, workId: null }), { scroll: false })
  }
  ...
  return <main className="pw-page pw-home">
    <PageHeader
      ...
      actions={<><button type="button" className="pw-secondary-action" onClick={product.refreshAll}>Refresh truth</button><button type="button" className="pw-primary-action" onClick={() => command.openCommand()}><Command size={14} /> Ask JARVIS</button></>}
    />
