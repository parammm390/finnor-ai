...
vi.mock("@/lib/centropy/proxy-auth", () => ({ getServiceToken: getServiceTokenMock }))
...
function request(method: string, path: string, options: { headers?: Record<string, string>; query?: string; body?: unknown } = {}): NextRequest {
  ...
  return new NextRequest(`http://localhost/api/centropy/${path}${query}`, {
    method,
...
describe("Phase 9 CENTROPY proxy boundary", () => {
  ...
  it.each([
    ["GET", CENTROPY_PROXY_READ_TIMEOUT_MS, "workspace-config"],
    ["POST", CENTROPY_PROXY_WRITE_TIMEOUT_MS, "semantic-activity"],
  ...
  it("turns upstream connection failure into a stable retryable response", async () => {
    ...
    await expect(response.json()).resolves.toEqual({ error: "Centropy backend is unavailable" })
  })
})
