// GET /api/centropy/stream?instructionId=... — centropy-v3 P3.T10 (plan v3 §7.1 Stage
// 2). A DEDICATED, non-buffering relay to finnor-os's real
// GET /api/stream?instructionId= (P3.T9) — deliberately NOT handled by the
// catch-all proxy (src/app/api/centropy/[...path]/route.ts), which `await
...
// this EXACT static path (`/api/centropy/stream`) in preference to the sibling
// catch-all's `[...path]` dynamic segment — the catch-all is never even invoked
// for this path, verified by e2e/centropy-stream-route.spec.ts's own assertion that
