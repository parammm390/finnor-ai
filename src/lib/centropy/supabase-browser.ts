...
// Browser-side Supabase client for real CENTROPY login (Phase 1.3). Uses the
 ...
 * login/reset action). Public CENTROPY preview therefore does not pay its cost
 * before rendering the non-authenticated Thread.
 */
export function getSupabaseBrowser(): Promise<SupabaseClient> {
  if (!isSupabaseBrowserConfigured()) {
    return Promise.reject(new Error("CENTROPY authentication is not configured for this deployment."))
  }
  if (!clientPromise) {
