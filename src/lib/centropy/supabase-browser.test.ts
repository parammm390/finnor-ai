...
describe("CENTROPY browser authentication configuration", () => {
  afterEach(() => {
    vi.unstubAllEnvs()
    vi.resetModules()
  })
  ...
  it("fails closed with an actionable error when public Supabase settings are absent", async () => {
    ...
    await expect(auth.getSupabaseBrowser()).rejects.toThrow("CENTROPY authentication is not configured for this deployment.")
  })
})
