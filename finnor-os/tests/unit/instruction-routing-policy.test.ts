...
const query = {
  route: "fast_read",
  confidence: "high",
  request: { intent: "deal_context", dealId: "00000000-0000-4000-8000-000000000014" },
} as const;
...
function action(overrides: Partial<DomainAction> = {}): DomainAction {
  return {
    ...
    payload: { recipient: { partyType: "external_contact", partyId: "00000000-0000-4000-8000-000000000013" }, channel: "email", body: "Please review the diligence item." },
    policyId: null,
    status: "draft",
...
describe("objective-first instruction-routing policy", () => {
  ...
  it("does not promote an unsupported read question into an objective", () => {
    expect(classifyInstructionRoute({
      instruction: "Tell me about industry benchmarks",
  ...
  it("reserves the atomic route for an exact one-effect candidate", () => {
    ...
    expect(finalizeInstructionRoute(preliminary, [action({ actionType: "send_message" })])).toMatchObject({
      route: "ATOMIC_EFFECT",
  ...
  it.each([
    "Resolve the financing workstream and keep going until it is verified",
    "Message the lender, wait for their reply, then update the closing condition",
    "Delegate this to the diligence lead and finish after they confirm completion",
    "Use the browser and then update the canonical deal record",
  ...
  it("rejects a nominally atomic instruction when the typed plan reveals a workflow or dependency", () => {
    ...
    expect(finalizeInstructionRoute(preliminary, [action({ compiledGraph: { kind: "workflow", commandType: "delegate_objective", requiresConfirmation: true, autoApprove: false } })]).route).toBe("OBJECTIVE");
