...
describe.skipIf(!available)("API dependency readiness", () => {
  ...
  it("separates liveness from migration/worker readiness and exposes no credential values", async () => {
    ...
    await getPool().query(`INSERT INTO service_release_heartbeats
      (service,instance_id,release_sha,build_id,version,release_source,core_certification_id,migration_head,capabilities,environment,cutover_protocol,product_epoch,last_beat_at)
      VALUES ('worker','ready-worker','test-sha','test-build','test-version','test','corecert-test',$1,ARRAY['jobs','orchestration'],'test',5,5,now())`,
